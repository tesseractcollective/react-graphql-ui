...
export interface IFlexFormProps {
  ...
  components?: Record<
    string,
    (props: ScalarComponentPropsBase) => ReactElement
  >;
...
function useFlexForm(props: IFlexFormProps) {
  ...
  const existingRecordQueryState = queryVariables
    ? useReactGraphql(config).useQueryOne({
        ...queryVariables,
        variables: {
  ...
  const data = useMemo(() => {
    ...
    for (const [datumName, datumValue] of Object.entries(data)) {
      const datumTypeName =
        config.fields?.fieldSimpleMap?.[datumName]?.typeName;
  ...
  let fields: string[] = useMemo(() => {
    ...
    const autoFields = map(
      config.fields?.fieldSimpleMap ?? {},
      (field) => field?.name ?? ''
    );
    ...
    return fields
      ...
      .map((field) => {
        const { fieldInfo, defaultComponent } = getDefaultComponentForScalar(
          config,
          field,
          rgUIContext.defaultComponents.flexFormComponents
        );
  ...
  return {
    formProps: {
      onSubmit: handleSubmit(props.onSubmit ?? onSubmit),
    ...
    fieldsElements,
    gqlError,
    errorElement: <small className="p-error p-d-block">{gqlError}</small>,
    submitButton: (
      <Button
